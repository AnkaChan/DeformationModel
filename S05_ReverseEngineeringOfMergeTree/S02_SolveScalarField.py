...
if __name__ == '__main__':
    ...
    constraintIds1 = contourLineConstraintOnInputCoords(coords, O1, r1, disThreshold=0.3)
    constraintIds2 = contourLineConstraintOnInputCoords(coords, O2, r2, disThreshold=0.3)
    contourLineConstraintIds = np.union1d(constraintIds1, constraintIds2)
    coordsOnCirlcle = coords[contourLineConstraintIds.tolist()]

    constraintImage = np.zeros((200, 200))
    for coord in coordsOnCirlcle:
        constraintImage[coord[0], coord[1]]= 255
    cv2.imwrite('ConstraintContourLine.png', constraintImage)
    ...
    for i, vId in enumerate(criticalPoints1D[:-2]):
    ...
    # for cId in contourLineConstraintIds:
    #     if cId not in inequilityConstraintIds:
    #         equalityConstraints.append(cId)
    #         equalityVal.append(contourLineHeight)


    A = np.zeros((len(equalityConstraints), numberOfViables)).astype(dType)
